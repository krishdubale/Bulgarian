...
class _WritingScreenState extends ConsumerState<WritingScreen>
  ...
  Widget build(BuildContext context) {
    ...
    final writingAsync = ref.watch(writingProvider(langId));

    return writingAsync.when(
      loading: () => Scaffold(
      ...
      error: (e, _) => Scaffold(
        appBar: AppBar(title: const Text('Writing Practice')),
        body: Center(child: Text('Error loading writing exercises: $e')),
      ),
      data: (exercises) {
        final translationExercises =
            exercises.where((e) => e.type == 'translate').toList();
        final fillBlankExercises =
            exercises.where((e) => e.type == 'fill_blank').toList();
        ...
        return Scaffold(
          ...
          body: TabBarView(
            controller: _tabController,
            children: [
              _buildExerciseTab(translationExercises, 'translate'),
              _buildExerciseTab(fillBlankExercises, 'fill_blank'),
            ],
          ),
        );
      },
  ...
  Widget _buildExerciseTab(List<WritingData> exercises, String type) {
    if (exercises.isEmpty) {
      return const Center(child: Text('No exercises available'));
    }

    if (_exerciseIndex >= exercises.length) {
      return _buildAllDone(exercises, type);
    }
  ...
  void _checkAnswer(WritingData exercise) {
    ...
    clean(String s) => s.replaceAll(RegExp(r'[.!?]$'), '').trim();
    final isCorrect = clean(userInput) == clean(correctAnswer);
    setState(() {
      _correct = isCorrect;
      _checked = true;
      if (isCorrect) _sessionScore++;
    });
  }

  Widget _buildAllDone(List<WritingData> exercises, String type) {
    final total = exercises.length;
    final pct = (_sessionScore / total * 100).round();
    final lessonId = 'writing_$type';
    final progress = ref.read(userProgressProvider);
    final alreadyCompleted =
        progress.completedLessons.contains(lessonId);

    if (!alreadyCompleted && _sessionScore > 0) {
      Future.microtask(() {
        ref
            .read(userProgressProvider.notifier)
            .addQuizXp(lessonId, _sessionScore, total, baseXp: 15);
      });
    }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              '$_sessionScore / $total correct ($pct%)',
